...
<cfquery name="GetUsers">
 SELECT LName,FName,UserName,Pass,Address,City,State,Zip,Phone
 FROM Users
 Order by LName Asc
</cfquery>
...
<cffile action="write"
             file="/var/www/gulfsportsnet.com/html/tnt-users.csv"
             output="Last Name,First Name,Email,Password,Address,City,State,Zip,Phone"
             addnewline="yes">
...
<!--- Output the contents of the export_contacts query --->
    <cfoutput>
        ...
        <cffile action="append"
                 file="/var/www/gulfsportsnet.com/html/tnt-users.csv"
                  output="#TRIM(Last)#,#TRIM(First)#,#TRIM(UserName)#,#TRIM(Pass)#,#TRIM(Add)#,#TRIM(City)#,#TRIM(State)#,#TRIM(Zip)#,#TRIM(Phone)#" 
                 addnewline="yes">
        </cfloop>
    </cfoutput>
...
<cflocation url="http://www.gulfsportsnet.com/tnt-users.csv">
</div>
</body>
</html>