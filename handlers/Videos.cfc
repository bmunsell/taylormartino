<cfcomponent name="Videos" extends="coldbox.system.EventHandler" output="false" accessors="true" autowire="true">
...
<!----------------------------------------- IMPLICIT EVENTS ------------------------------------------>
	...
	<cffunction name="preHandler" returntype="void" output="false" hint="Gets the Services">
		...
		<cfscript>
			...
			//Get Videos
			prc.user = yt.getVideosByUser('taylormartino');
		</cfscript>
...
<!------------------------------------------- PUBLIC EVENTS ------------------------------------------>
	...
	<cffunction name="index" returntype="void" output="false" hint="My main event">
		...
		<cfargument name="prc">
		<cfscript>
			event.paramValue("p",0);
			event.paramValue("y",0);
			
			//Page
			prc.currentpage = pageService.Getpage(PageDir=rc.Page.PageDir);	
			//Slideshow
			prc.photos = pageService.GetPhotos(GalleryID=4);	
			//GET ALL Subpages
			prc.subpages = pageService.GetAllSubPages(PageID=prc.currentpage.pageid);
			
			prc.vi = Right(prc.user.id, 11);
			if(rc.y) {            
            	prc.foo = "http://gdata.youtube.com/feeds/api/videos/#prc.vi#";
				prc.i = yt.getVideo(prc.foo);		            
            } 
			
			
			prc.theURL = "http://www.myfoxal.com/category/151717/sports?clienttype=rss&redirected=true";

			prc.entries = rss.getEntries(prc.theURL);
	...
	<cffunction name="detail" output="false" hint="index">
		<cfargument name="event">
		<cfargument name="rc">
		<cfargument name="prc">
		<cfscript>
		event.paramValue("id","");
		prc.currentpage = pageService.Getpage(PageDir='videos');
		
		prc.feed = "http://gdata.youtube.com/feeds/api/videos/#rc.id#";
		prc.video = yt.getVideo(prc.feed);
		
		rc.title = "#prc.video.title# - video by Taylor Martino";
		rc.description = "#prc.video.description#";
		rc.keywords = "#prc.video.keywords#";
		rc.fimage = "#prc.video.THUMBNAIL_URL#";
		
		event.setView("videos/detail");
		</cfscript>
	</cffunction>
