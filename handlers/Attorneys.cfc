<cfcomponent name="Attorneys" extends="coldbox.system.EventHandler" output="false" accessors="true" autowire="true">
...
<!------------------------------------------- PUBLIC EVENTS ------------------------------------------>
	...
	<cffunction name="index" output="false" hint="index">
		...
		<cfargument name="prc">
		<cfscript>
			event.paramValue("Page.PageDir","attorneys");
			try {                           
            	prc.currentpage = pageService.Getpage(PageDir='attorneys');                       
            }                                    
            catch(any e) {                                    
            	getPlugin("MessageBox").setMessage(type="error", message="We're sorry an error has occurred or this page does not exist.");                        
            	event.setView("404");                        
            }
			if (!isDefined('prc.currentpage.recordcount') OR prc.currentpage.recordcount LT 1) {
				getPlugin("MessageBox").setMessage(type="error", message="We're sorry an error has occurred or this page does not exist.");                        
            	event.setView("404");  
			} else {
				event.setView("attorneys/index");
			}
...
	<cffunction name="attorney" returntype="void" output="false" hint="My main event">
		<cfargument name="event">
		<cfargument name="rc">
		<cfargument name="prc">
		<cfscript>
			event.paramValue("Page.PageDir","attorneys");
			prc.mName = 'Back';
		 	prc.mLink = '#getSetting('websiteurl')#/attorneys';
			prc.parent = pageService.Getpage(PageDir='attorneys');
			try {                           
			...
			if (!isDefined('prc.subpage.recordcount') OR prc.subpage.recordcount LT 1) {
				...
				event.setView("attorneys/detail");
			}
