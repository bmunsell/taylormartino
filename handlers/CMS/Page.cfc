...
<!------------------------------------------- PUBLIC EVENTS ------------------------------------------>
	...
	<cffunction name="add" output="false" hint="index">
		...
		<cfscript>
			...
			prc.getTypes = pageService.GetPageTypes();
			var local = StructNew();
		...
		<cfif(IsDefined("rc.MM_InsertRecord") AND rc.MM_InsertRecord EQ "form1")>
			...
			<cfif IsDefined("rc.Photo") AND #rc.Photo# NEQ "">
				...
				<cffile action="upload" destination="#getSetting('imagepath')#" filefield="Photo" nameconflict="makeunique" >
				...
				<cfset rc.pageText = '<h1>#rc.pageTitle#</h1>' & 
				'<ul class="gallery clearfix"><li>' & 
				'<a href="/inc/images/#local.FileName#" rel="prettyPhoto" title="#rc.pageTitle#">' &
				'<img src="/inc/images/#local.thumbName#" alt="#rc.pageTitle#" />' &
				'</a></li></ul><p>Sample Text. Delete me.</p>'>
			</cfif>
			<cfscript>
				prc.theid = createService.createPage(
				...
				location(url="/cms/page/edit/?pid=#prc.theid.GENERATED_KEY#",addToken='no');
			</cfscript>
		</cfif>
	</cffunction>
	...
	<cffunction name="edit" output="false" hint="index">
		...
		<cfscript>
			...
			//Get Page
			prc.getPage = pageService.GetAllPages(pageID=rc.pid);
			
			if (not prc.getPage.recordcount){
				location(url="/cms/page/list/?error=1",addToken='no');
			}
			//Form Results
